...
const PutOnSale = () => {
  ...
  const fetchBooks = async () => {
    try {
      const response = await axios.get('https://bookmart-d7it.onrender.com/api/books');
  ...
  const handlePutOnSale = async (bookId) => {
    try {
      ...
      await axios.patch(
        `https://bookmart-d7it.onrender.com/api/books/${bookId}/sale`,
        { discount: discount },
        {
          headers: {
  ...
  const handleRemoveSale = async (bookId) => {
    try {
      await axios.patch(
        `https://bookmart-d7it.onrender.com/api/books/${bookId}/remove-sale`,
        {},
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
